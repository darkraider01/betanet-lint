...
use goblin::{mach::Mach, Object, elf};
...
impl BinaryMeta {
    pub fn from_path(path: PathBuf) -> Result<Self, String> {
        ...
        let file_size = file.metadata().map_err(|e| format!("metadata error: {e}"))?.len();
        ...
        // SHA256 with streaming for large files
        let sha256 = if file_size > 100 * 1024 * 1024 {
            compute_sha256_streaming(&path)?
        } else {
        ...
        // Extract strings with memory limits
        let strings = extract_ascii_strings_optimized(&raw, 4);

        // Enhanced binary analysis with symbol extraction
        let (format, needed_libs, imported_symbols, exported_symbols, section_names, dynamic_dependencies) = 
            analyze_binary_format(&raw)?;
        ...
        let betanet_indicators = detect_betanet_indicators_enhanced(
            &strings, &needed_libs, &imported_symbols, &exported_symbols, 
            &section_names, &dynamic_dependencies
        );
        let build_reproducibility = analyze_build_reproducibility(&raw, &format, &strings);
...
fn compute_sha256_streaming(path: &PathBuf) -> Result<String, String> {
    let mut file = File::open(path).map_err(|e| format!("stream open error: {e}"))?;
    let mut hasher = Sha256::new();
...
/// Enhanced binary format analysis with symbol extraction
fn analyze_binary_format(raw: &[u8]) -> Result<(BinFormat, Vec<String>, Vec<String>, Vec<String>, Vec<String>, Vec<String>), String> {
    match Object::parse(raw).map_err(|e| e.to_string())? {
        Object::Elf(elf) => {
        ...
        Object::Mach(Mach::Binary(m)) => {
            ...
            // Extract imported symbols from Mach-O
        ...
        Object::PE(pe) => {
            ...
            // Extract imported symbols from PE
            let imported_symbols: Vec<String> = pe.imports
                .iter()
                .flat_map(|import| {
                    // No direct equivalent to import_lookup_table in goblin 0.7 for PE imports
                    // We need to iterate over the import descriptors and get their names
                    // This is a simplified approach, a more robust solution would
                    // involve parsing the import directory entries more thoroughly.
                    // For now, we'll just use the DLL name.
            ...
            let exported_symbols: Vec<String> = pe.exports
                .iter()
                .map(|export| export.name.unwrap_or("").to_string())
                .collect();

            // Extract section names
            let section_names: Vec<String> = pe.sections
                .iter()
                .map(|section| section.name().unwrap_or("").to_string())
                .collect();
...
fn extract_ascii_strings_optimized(buf: &[u8], min_len: usize) -> Vec<String> {
    ...
    let max_strings = 50000; // Reduced from 100k
    let max_string_length = 512; // Reduced from 1KB
    ...
    for string in strings.iter().take(1000) { // Limit search
        if string.contains("rustc") {
            return Some(CompilerInfo {
                ...
                optimization_level: extract_opt_level(string).unwrap_or("unknown".to_string()),
                target_triple: extract_target_triple(string).unwrap_or("unknown".to_string()),
            });
...
fn extract_version(string: &str, tool: &str) -> Option<String> {
    let re = regex::Regex::new(&format!(r"{tool}\s+(\d+\.\d+\.\d+)")).ok()?;
...
fn detect_build_environment() -> BuildEnvironment {
    ...
    BuildEnvironment {
        ...
        build_timestamp: env::var("SOURCE_DATE_EPOCH").ok()
            .map(|epoch| {
                chrono::DateTime::from_timestamp(epoch.parse().unwrap_or(0), 0)
...
fn detect_crypto_components(strings: &[String], libs: &[String]) -> Vec<CryptographicComponent> {
    ...
    // Betanet 1.1 required crypto primitives
    let crypto_patterns = [
        ("ChaCha20-Poly1305", Some(256), Some("IETF"), true),
        ("SHA-256", Some(256), None, true),
        ("HKDF-SHA256", Some(256), None, true),
        ("Ed25519", Some(255), None, true),
        ("X25519", Some(255), None, true),
        ("X25519-Kyber768", Some(768), Some("hybrid"), true),
        ("AES", Some(256), Some("GCM"), true),
        ("RSA", Some(2048), None, false), // Deprecated
    ];
    
    for (algo, key_len, mode, quantum_safe) in crypto_patterns {
        if strings.iter().any(|s| s.to_lowercase().contains(&algo.to_lowercase().replace('-', ""))) {
    ...
    for lib in libs {
        if lib.contains("ring") || lib.contains("rustls") {
            components.push(CryptographicComponent {
                algorithm: "Rust Crypto".to_string(),
                key_length: None,
                mode: None,
...
fn detect_betanet_indicators_enhanced(
    ...
    // Symbol-based protocol detection (more reliable than strings)
    let crypto_symbols = [
    ...
    // Enhanced string analysis with context
...
fn analyze_build_reproducibility(raw: &[u8], _format: &BinFormat, strings: &[String]) -> BuildReproducibility {
    ...
    // Proper build ID extraction
    match Object::parse(raw) {
        Ok(Object::Elf(elf)) => {
            // Check for GNU build ID in note sections
            ...
            // Fallback: check for build ID section
            if !has_build_id {
                for section in &elf.section_headers {
                    if let Some(name) = elf.shdr_strtab.get_at(section.sh_name) {
                        if name == ".note.gnu.build-id" {
                            ...
                            // In goblin 0.7, section data needs to be accessed directly from the raw binary.
                            let section_offset = section.sh_offset as usize;
                            let section_size = section.sh_size as usize;
                            if raw.len() >= section_offset + section_size {
        ...
        Ok(Object::Mach(goblin::mach::Mach::Binary(m))) => {
            // Check for UUID load command in Mach-O
        ...
        Ok(Object::PE(pe)) => {
            // Check for debug directory in PE
            // PE debug data parsing is temporarily commented out due to API changes in `goblin`
            // and to avoid `unused_variables` warnings.
            // Re-enable and adapt when `goblin` is updated or a robust workaround is found.
            // if let Some(debug_data) = &pe.debug_data {
            //     // Original code for debug data processing
            // }
            
            // Fallback: check timestamp
            if !has_build_id && pe.header.coff_header.time_date_stamp != 0 {
                has_build_id = true;
